...
O projeto `devops-ia` precisa de uma pipeline de CI/CD no GitHub Actions para automatizar o build e push de imagens Docker para o ECR. A autenticacao do GitHub Actions na AWS e um passo fundamental que deve ser seguro, auditavel e seguir o principio de least privilege.

Atualmente, o projeto possui:
- **Cluster EKS** (`devops-ia-production`) na regiao `us-east-1`, provisionado pela stack `02-eks-stack-ai`
- **Repositorios ECR** para `frontend` e `backend` com nomes `devops-ia/production/frontend` e `devops-ia/production/backend`
...
- **Terraform**: >= 1.10.0
- **Backend S3**: `devops-ia-production-terraform-state`
- **Repositorio GitHub**: `SalesFX/aws-devops-platform`
...
- **Restricao de federacao**: A trust policy deve restringir o assume role exclusivamente ao repositorio `SalesFX/aws-devops-platform` e a branch `main`
...
2. **Security**: Zero credenciais de longa duracao. Trust policy restrita a `repo:SalesFX/aws-devops-platform:ref:refs/heads/main`. Tokens OIDC tem vida util de minutos. Cada execucao e auditavel via CloudTrail com o claim `sub` do token.
...
- **Riscos e mitigacoes**:
  ...
  - *Risco*: Trust policy excessivamente permissiva. *Mitigacao*: Condition restrita a `repo:SalesFX/aws-devops-platform:ref:refs/heads/main` -- nao aceita forks, PRs de forks, ou outras branches.
...
graph TB
    subgraph "AWS Account 654654554686"
        ...
        IAM_ROLE["IAM Role<br/>devops-ia-github-actions-role"]
        IAM_POLICY["IAM Policy<br/>ECR Push + Login"]
        ECR_BE["ECR: devops-ia/production/backend"]
        ECR_FE["ECR: devops-ia/production/frontend"]
    ...
    subgraph "GitHub"
        REPO["SalesFX/aws-devops-platform"]
...
- **Nova stack**: `devops-ia-terraform/03-ci-cd-stack-ai/`
...
- **Trust Policy da IAM Role** (condicoes criticas):
  ...
  - `StringLike` em `token.actions.githubusercontent.com:sub` = `repo:SalesFX/aws-devops-platform:ref:refs/heads/main`
  ...
  ```json
  {
    ...
    "Statement": [
      ...
      },
      {
        ...
        "Resource": [
          "arn:aws:ecr:us-east-1:654654554686:repository/devops-ia/production/backend",
          "arn:aws:ecr:us-east-1:654654554686:repository/devops-ia/production/frontend"
        ]
      }
    ]
  }
...
- **IAM (least privilege)**:
  - Role restrita a ECR push apenas nos repositorios `devops-ia/production/backend` e `devops-ia/production/frontend`
