...
O projeto `devops-ia` ja possui uma stack de networking (`01-networking-stack-ai`) com VPC, subnets publicas/privadas, NAT Gateway e route tables provisionadas na regiao `us-east-1`. O proximo passo natural e provisionar um cluster Kubernetes gerenciado (EKS) para executar workloads containerizados.
...
- **Backend**: S3 bucket `devops-ia-production-terraform-state` com lock file e encriptacao
...
eks = {
  ...
  node_group = {
    name           = "devops-ia-production-general"
...
data "terraform_remote_state" "networking" {
  backend = "s3"
  config = {
    bucket = "devops-ia-production-terraform-state"
...
2. `aws eks describe-cluster --name devops-ia-production --region us-east-1` -- verificar status `ACTIVE`
3. `aws eks describe-nodegroup --cluster-name devops-ia-production --nodegroup-name devops-ia-production-general --region us-east-1` -- verificar status `ACTIVE` e 2 nodes
4. `aws eks update-kubeconfig --name devops-ia-production --region us-east-1` -- configurar kubectl
...
- CloudWatch Logs volume para `/aws/eks/devops-ia-production/cluster`
...
### Logging e auditoria
- **EKS Control Plane Logs**: todos os 5 tipos habilitados, enviados para CloudWatch Logs group `/aws/eks/devops-ia-production/cluster`
