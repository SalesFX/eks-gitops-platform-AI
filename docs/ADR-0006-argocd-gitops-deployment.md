...
O projeto `devops-ia` precisa de uma ferramenta de Continuous Delivery que implemente o padrao GitOps para deployer automaticamente as aplicacoes frontend e backend no cluster EKS (`devops-ia-production`, regiao `us-east-1`, Kubernetes 1.31).
...
- **EKS**: `devops-ia-production`, versao 1.31, regiao `us-east-1`
- **Node group**: 2x `t3.medium` ON_DEMAND, subnets privadas, disk 20GB
- **Addons instalados**: vpc-cni, coredns, kube-proxy
- **Manifestos**: Kustomize com `kustomization.yaml` em `devops-ia-kubernetes/`
...
- **ECR**: Repositorios `devops-ia/production/backend` e `devops-ia/production/frontend`

### Constraints levantados no discovery

- **Repositorio unico**: O mesmo repositorio (`SalesFX/aws-devops-platform`) contem codigo das apps, manifestos Kubernetes e IaC -- monorepo
...
- **Descricao**: Instalar o ArgoCD no cluster EKS usando os manifests oficiais de instalacao (namespace `argocd`), e configurar um recurso `Application` que monitora o diretorio `devops-ia-kubernetes/` do repositorio GitHub. O ArgoCD detecta mudancas no `kustomization.yaml` e faz sync automatico.
...
flowchart TD
    subgraph "GitHub Repository"
        KUSTOM["devops-ia-kubernetes/<br/>kustomization.yaml<br/>newTag: sha-abc1234"]
        MANIFESTS["backend/ + frontend/<br/>deployment.yaml<br/>service.yaml<br/>pdb.yaml"]
    end

    subgraph "EKS Cluster: devops-ia-production"
        subgraph "Namespace: argocd"
            ...
            ARGO_APP["Application CRD<br/>devops-ia"]
        end
...
sequenceDiagram
    ...
    Argo->>K8s: kubectl apply -k devops-ia-kubernetes/
...
- **Application CRD** -- criar arquivo `devops-ia-kubernetes/argocd-application.yaml`:
  ```yaml
  ...
  spec:
    project: default
    source:
      repoURL: https://github.com/SalesFX/aws-devops-platform.git
      targetRevision: main
...
- **Configuracoes-chave da Application**:
  - `source.path`: `devops-ia-kubernetes` (diretorio com kustomization.yaml)
...
- **Repositorio**: Como o repositorio `SalesFX/aws-devops-platform` e publico, nao e necessario configurar credenciais. Se fosse privado:
  ```
  argocd repo add https://github.com/SalesFX/aws-devops-platform.git --username <user> --password <token>
  ```

- **Ordem de execucao e dependencias**:
  1. Stack 02 (EKS) provisionada e acessivel
  2. ADR-0004 (OIDC + IAM) implementada (stack 03)
  3. `kubectl` configurado para o cluster: `aws eks update-kubeconfig --name devops-ia-production --region us-east-1`
...
- **Validacoes pos-deploy**:
  ...
  - Application status: `kubectl get application devops-ia -n argocd`
  - Sync status: `argocd app get devops-ia` (via CLI) ou UI web
  - Health status de todos os recursos: `argocd app get devops-ia --show-health`

- **Rollback strategy**:
  - Via Git: `git revert <commit>` no kustomization.yaml -- ArgoCD detecta e aplica
  - Via ArgoCD CLI: `argocd app rollback devops-ia <history-id>`
...
- **Runbooks necessarios**:
  - Procedimento para forcar sync manual: `argocd app sync devops-ia`
  - Procedimento para rollback via ArgoCD: `argocd app rollback devops-ia`
  - Procedimento para limpar Application stuck: `argocd app delete devops-ia --cascade`
