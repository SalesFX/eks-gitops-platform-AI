...
O repositorio `aws-devops-platform` e um workshop AWS com multiplas stacks Terraform independentes, prefixadas com dois digitos (ex: `01-networking-stack-ai`). Atualmente, o state de cada stack e armazenado localmente (`terraform.tfstate`), o que apresenta riscos de perda de dados, impossibilita colaboracao em equipe e nao oferece mecanismos de locking para prevenir modificacoes concorrentes.
...
- **Terraform**: `>= 1.10.0`
- **Regiao**: `us-east-1`
- **Projeto**: `devops-ia`, ambiente `production`
- **Naming**: projeto segue convencao `devops-ia-production` para nomes de recursos
...
graph TB
    subgraph "Stack: remote-backend"
        S3["S3 Bucket<br/>devops-ia-production-terraform-state"]
...
```
devops-ia-production-terraform-state/
...
backend = {
  bucket_name                        = "devops-ia-production-terraform-state"
